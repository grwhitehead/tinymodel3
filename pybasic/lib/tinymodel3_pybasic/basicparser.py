...
class BASICParser:
    ...
    def __simplestmt(self):
        ...
        elif self.__token.category == Token.RANDOM:
            self.__randomstmt()
            return None
        ...
        elif self.__token.category == Token.SET:
            self.__setstmt()
            return None

        elif self.__token.category == Token.RESET:
            self.__resetstmt()
            return None

        elif self.__token.category == Token.POKE:
            self.__pokestmt()
            return None
    ...
    def __inputstmt(self):
        ...
        while not valid_input:
            ...
            inputvals = self.runtime.input(prompt).split(',', (len(variables)-1))
    ...
    def __evaluate_function(self, category):
        ...
        if category == Token.POINT:
            ...
            return self.runtime.point(x,y)

        if category == Token.PEEK:
            self.__consume(Token.LEFTPAREN)

            self.__expr()
            addr = self.__operand_stack.pop()

            self.__consume(Token.RIGHTPAREN)

            return self.runtime.peek(int(addr))
    ...
    def __randomstmt(self):
        """Implements a function to seed the random
        number generator

        """
        self.__advance()  # Advance past RANDOM token
    ...
    def __printstmt(self):
        ...
        if self.__token.category == Token.AT:
            # Process the @ keyword
            self.__consume(Token.AT)

            # Acquire the position
            self.__expr()
            pos = int(self.__operand_stack.pop())

            if pos < 0 or pos >= self.runtime.chars_len:
                raise RuntimeError("PRINT: @"+str(pos)+" invalid screen position in line " + str(self.__line_number))

            self.runtime.cursor = pos

            # Process the comma
            if self.__tokenindex < len(self.__tokenlist) and self.__token.category != Token.COLON:
                self.__consume(Token.COMMA)

        while self.__tokenindex < len(self.__tokenlist):
            if self.__token.category == Token.SEMICOLON:
                self.__advance()

                # don't print a newline if we end with a semicolon
                if self.__tokenindex == len(self.__tokenlist):
                    return
                
            elif self.__token.category == Token.COMMA:
                self.__advance()
                
                # advance to next 16 char print zone
                current_pr_column = self.runtime.cursor%self.runtime.line_width
                self.__prnt_column = current_pr_column + 16 - current_pr_column%16
                if current_pr_column < self.__prnt_column:
                    self.runtime.print(" "*(self.__prnt_column-current_pr_column))
                        
            elif self.__token.category == Token.TAB:
                self.__logexpr()

                # advance to specified tab position
                current_pr_column = self.runtime.cursor%self.runtime.line_width
                self.__prnt_column = len(self.__operand_stack.pop())
                if current_pr_column < self.__prnt_column:
                    self.runtime.print(" "*(self.__prnt_column-current_pr_column))
                ...
                self.__prnt_column += len(valstr)
                self.runtime.print(valstr)
    ...
    def __clsstmt(self):
        """Parses a cls, clears the screen
        """
    ...
    def __setstmt(self):
        """Parses a set statement, sets a pixel on the screen
        """

        self.__advance() # Advance past SET token
        ...
        self.runtime.set(x,y)

    def __resetstmt(self):
        """Parses a reset statement, resets a pixel on the screen
        """

        self.__advance() # Advance past RESET token
        ...
        self.runtime.reset(x,y)

    def __pokestmt(self):
        """Parses a poke statement, stores a value in memory
        """

        self.__advance() # Advance past POKE token
        ...
        self.runtime.poke(int(addr), int(val))
